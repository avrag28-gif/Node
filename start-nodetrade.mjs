...
function start(cmd, args, name, env = {}) {
  const p = spawn(cmd, args, { cwd: ROOT, env: { ...process.env, ...env }, stdio: 'inherit', windowsHide: false });
  children.push(p);
  p.on('exit', (code, signal) => console.log(`[NodeTrade] ${name} exited code=${code} signal=${signal || ''}`));
  return p;
}
...
function request(port, method, path, headers = {}, body = null) {
  return new Promise((resolve, reject) => {
    const h = { ...headers, host: `127.0.0.1:${port}`, connection: 'close' };
...
async function ai(path, method, payload) {
  const body = payload == null ? null : JSON.stringify(payload);
  const r = await request(AI_PORT, method, path, { 'content-type': 'application/json' }, body);
...
async function ngrokUrl() {
  try {
    const r = await request(4040, 'GET', '/api/tunnels');
    const j = JSON.parse(r.body.toString('utf8'));
    return j.tunnels?.find(t => t.public_url?.startsWith('https://'))?.public_url || j.tunnels?.[0]?.public_url || null;
  } catch { return null; }
}
...
async function handle(req, res) {
  ...
  if (req.url === '/api/market/train' && req.method === 'POST') {
    ...
    const started = await ai('/train', 'POST', {
      symbol: 'XAUUSD',
      timeframe: payload.timeframe || '15m',
      startDate: payload.startDate || null,
      endDate: payload.endDate || null,
      epochs: Math.max(1, Math.min(100, Number(payload.epochs) || 20)),
      horizon: 10,
    });
  ...
  if (req.url === '/api/market/training-status' && req.method === 'GET') {
    const s = await ai('/training/status', 'GET', null);
    const d = s.data || {};
    return sendJson(res, s.status, {
      isTraining: !!d.isTraining,
      currentEpoch: d.currentEpoch || 0,
      totalEpochs: d.totalEpochs || 0,
      currentLoss: d.currentLoss ?? null,
      currentAccuracy: d.currentAccuracy ?? null,
      status: d.status || 'idle',
      hasTrainedModel: !!d.summary,
  ...
  if (req.url === '/v1/analyze' && req.method === 'POST') {
    const node = await request(NODE_PORT, req.method, req.url, req.headers, body);
    let original;
  ...
  if (req.url === '/api/market/analyze' && req.method === 'POST') {
    ...
    if (Array.isArray(data.candles) && data.candles.length) {
      const pred = await ai('/predict', 'POST', { symbol: 'XAUUSD', timeframe: '15m', candles: data.candles });
      if (pred.status < 400 && pred.data?.model_ready) data.signal = { ...data.signal, ...pred.data };
    }
  ...
  if (req.url === '/api/status' && req.method === 'GET') {
    ...
    if (url) { data.publicUrl = url; data.customDomain = url; }
    return sendJson(res, node.status, data);
  }
...
function sendRaw(res, r) {
  for (const [k, v] of Object.entries(r.headers)) if (v !== undefined && k !== 'content-length' && k !== 'transfer-encoding') res.setHeader(k, v);
...
const server = http.createServer((req, res) => {
  handle(req, res).catch(err => {
    console.error('[NodeTrade proxy]', err);
