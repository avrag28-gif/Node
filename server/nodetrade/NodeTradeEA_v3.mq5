...
input string InpAiServiceOrigin = "http://127.0.0.1:8010";
...
bool Get(string base,string path,string &out){
   ...
   if(code<200||code>=300)return false; out=CharArrayToString(result,0,ArraySize(result),CP_UTF8); return true;
}

bool Activate(){ string body=StringFormat("{\"account_id\":\"%s\",\"activation_key\":\"%s\"}",Esc(AccountID()),Esc(InpActivationCode)); string r=""; if(!Post(InpServerOrigin,"/v1/activate",body,false,r))return false; token=JStr(r,"token"); connected=(token!=""); return connected; }
void Heartbeat(){ if(token=="")return; string r=""; string body=StringFormat("{\"account_id\":\"%s\",\"symbol\":\"%s\",\"terminal_time\":%d}",Esc(AccountID()),Esc(_Symbol),(long)TimeCurrent()); if(Post(InpServerOrigin,"/v1/heartbeat",body,true,r))lastHeartbeat=TimeCurrent(); }
...
void IngestLive(){
   ...
   string body=StringFormat("{\"symbol\":\"%s\",\"timeframe\":\"%s\",\"candles\":%s}",Esc(_Symbol),TfName((ENUM_TIMEFRAMES)Period()),CandleJson(rates,n)); string r=""; Post(InpAiServiceOrigin,"/ingest",body,false,r);
}

void PollTrainingRequest(){
   string r=""; if(!Get(InpAiServiceOrigin,"/training/request",r))return; if(!JBool(r,"pending"))return;
   ...
   for(int pos=0;pos<n;pos+=chunk){
      ...
      if(!Post(InpAiServiceOrigin,"/ingest",body,false,out)){ Print("[NodeTrade] Upload training chunk gagal"); return; }
   }
   string clear=""; Post(InpAiServiceOrigin,"/training/request/clear","{}",false,clear);
...
void Analyze(){
   ...
   string r=""; if(!Post(InpServerOrigin,"/v1/analyze",body,true,r))return;
...
int OnInit(){ EventSetTimer(MathMax(1,InpTimerSeconds)); Activate(); return INIT_SUCCEEDED; }
