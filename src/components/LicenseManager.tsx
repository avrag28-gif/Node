      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {licenses.length === 0 ? (
          <div className="col-span-2 py-8 text-center text-slate-500 text-xs italic">
            Belum ada lisensi MT5 yang dibuat. Klik "New License" untuk menambahkan akun.
          </div>
        ) : (
          licenses.map((lic) => {
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center space-x-2">
                    <ShieldCheck
                      className={`w-4 h-4 ${
                        lic.enabled ? "text-emerald-400" : "text-slate-500"
                      }`}
                    />
                    <span className="text-xs font-bold text-slate-200">
                      {lic.label || "Account"}
                    </span>
                    <span className="text-[11px] font-mono text-slate-400 bg-slate-900 px-1.5 py-0.2 rounded border border-slate-800">
                      ID: {lic.account_id}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center gap-1">
                    <button
                      onClick={() => onToggle(lic.account_id)}
                      title={lic.enabled ? "Disable" : "Enable"}
                    ...
                    <button
                      onClick={() => onDelete(lic.account_id)}
                      title="Delete"
                ...
                <div className="bg-slate-900 rounded p-2 border border-slate-800 flex items-center justify-between text-xs font-mono">
                  ...
                  <button
                    onClick={() => handleCopy(lic.activation_key, lic.account_id)}
                    className="text-slate-400 hover:text-emerald-400 ml-2 transition flex items-center shrink-0"
                    title="Copy Key"
                  >
                    {isCopied ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              <div>
                ...
                <input
                  type="text"
                  required
                  placeholder="e.g. 5291823"
                  value={newAccountId}
                  onChange={(e) => setNewAccountId(e.target.value)}
                  className="w-full bg-slate-950 border border-slate-800 rounded-lg px-3 py-1.5 text-xs text-slate-100 font-mono focus:border-emerald-500 outline-none"
                />
              </div>
              ...
              <div>
                ...
                <input
                  type="text"
                  placeholder="e.g. VPS / Client Account"
                  value={newLabel}
                  onChange={(e) => setNewLabel(e.target.value)}
                  className="w-full bg-slate-950 border border-slate-800 rounded-lg px-3 py-1.5 text-xs text-slate-100 focus:border-emerald-500 outline-none"
                />
              </div>
