...
function start(cmd, args, name, env = {}) {
  ...
  console.log(`[NodeTrade] Starting ${name}: ${cmd} ${args.join(' ')}`);
  const p = spawn(cmd, args, spawnOptions);
  children.push(p);
  p.on('exit', (code, signal) => console.log(`[NodeTrade] ${name} exited code=${code} signal=${signal || ''}`));
  p.on('error', err => console.error(`[NodeTrade] ${name} spawn error:`, err));
  return p;
}

if (!existsSync(PY)) throw new Error(`Python venv not found: ${PY}`);

// On Windows, npm.cmd must be spawned through cmd.exe to avoid Node 24 spawn EINVAL.
...
start(npmCommand, npmArgs, 'Node server');
...
async function handle(req, res) {
  ...
  if (req.url === '/api/market/train' && req.method === 'POST') {
    ...
    while (Date.now() < deadline) { const q = await ai('/training/request', 'GET', null); if (!q.data?.pending) break; await new Promise(r => setTimeout(r, 1000)); }
    ...
    while (Date.now() < trainDeadline) { const s = await ai('/training/status', 'GET', null); if (!s.data?.isTraining) { const summary = s.data?.summary || null; if (s.data?.status === 'error') return sendJson(res, 500, { error: s.data.error || 'AI training failed' }); return sendJson(res, 200, { trainingSummary: summary, status: 'completed', source: 'MT5 → CSV → Python Ensemble' }); } await new Promise(r => setTimeout(r, 1000)); }
...
server.on('error', err => { console.error('[NodeTrade gateway] server error:', err); process.exitCode = 1; });
server.listen(PUBLIC_PORT, '0.0.0.0', () => console.log(`[NodeTrade] Public gateway listening on http://0.0.0.0:${PUBLIC_PORT}`));
process.on('uncaughtException', err => console.error('[NodeTrade] uncaughtException:', err));
process.on('unhandledRejection', err => console.error('[NodeTrade] unhandledRejection:', err));
function shutdown() { for (const p of children) { try { p.kill(); } catch {} } server.close(() => process.exit(0)); }
