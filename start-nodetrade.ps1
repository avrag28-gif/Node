...
function Require-Cmd($name) {
  if (-not (Get-Command $name -ErrorAction SilentlyContinue)) { throw "$name tidak ditemukan. Install dulu lalu jalankan lagi." }
}
...
if (Get-Command py -ErrorAction SilentlyContinue) {
  try { & py -3.11 --version *> $null; if ($LASTEXITCODE -eq 0) { $python = 'py -3.11' } } catch {}
}
...
if (-not (Get-Command ngrok -ErrorAction SilentlyContinue)) {
  Write-Host 'ngrok belum ada. Mencoba install via winget...' -ForegroundColor Yellow
  if (Get-Command winget -ErrorAction SilentlyContinue) {
    winget install ngrok -s msstore --accept-package-agreements --accept-source-agreements
  } else { throw 'ngrok tidak ditemukan dan winget tidak tersedia.' }
}
Require-Cmd 'ngrok'

if (-not (Test-Path '.venv\Scripts\python.exe')) {
  Write-Host 'Membuat Python venv...' -ForegroundColor Yellow
...
try {
  ...
  if ($url) {
    Write-Host "`nNodeTrade PUBLIC URL: $url" -ForegroundColor Green
    Write-Host "MT5 WebRequest URL:  $url" -ForegroundColor Green
    Write-Host "Dashboard:           $url" -ForegroundColor Green
  } else { Write-Host 'ngrok hidup tetapi URL belum tersedia.' -ForegroundColor Yellow }
} catch {
  Write-Host 'ngrok belum mengembalikan URL. Cek http://127.0.0.1:4040.' -ForegroundColor Yellow
}
